    ...
    return (
        <div className="bg-black border border-green-800 p-4">
            <div className="flex items-center justify-between mb-3">
                <span className="text-green-500 text-xs font-mono uppercase">TRUST_MATRIX</span>
                <span className="text-green-500 text-xs font-mono">{threatLevel}/100</span>
            </div>
            <div className="flex gap-1">
                {Array.from({ length: blocks }).map((_, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={`h-8 flex-1 border transition-all duration-300 ${isFilled
                                    ? isHighThreat
